                ...
                <form action="{{ route('admin.reels.update', $reel->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="card-body card-padding">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $reel->title) }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="youtube_id" class="form-control @error('youtube_id') is-invalid @enderror" value="{{ old('youtube_id', $reel->youtube_id) }}" placeholder="e.g. F8VDoF5WB3Y">
                            <small class="text-muted">If provided, YouTube video is used. Leave blank to use uploaded MP4.</small>
                        ...
                        <div class="form-group">
                            ...
                            @if($reel->video_file)
                                <p class="text-muted mb-1">Current: <strong>{{ $reel->video_file }}</strong></p>
                            @endif
                            <input type="file" name="video_file" accept="video/mp4,video/webm" class="form-control @error('video_file') is-invalid @enderror">
                            <small class="text-muted">Leave blank to keep existing. Max 100MB.</small>
                        ...
                        <div class="form-group">
                            @php
                                $currentThumb = $reel->youtube_id
                                    ? "https://img.youtube.com/vi/{$reel->youtube_id}/maxresdefault.jpg"
                                    : ($reel->thumbnail_image ? asset('admin_assets/uploads/' . $reel->thumbnail_image) : null);
                            @endphp
                            @if($currentThumb)
                                <div id="image-preview" class="mb-4 profile_image_box" style="display:block;">
                                    <img id="preview-img" src="{{ $currentThumb }}" class="img-thumbnail" alt="Thumbnail" style="max-width:120px;">
                                </div>
                            @else
                        ...
                        <div class="row">
                            <div class="col-md-4 form-group">
                                ...
                                <input type="number" step="0.01" name="price" class="form-control @error('price') is-invalid @enderror" value="{{ old('price', $reel->price) }}">
                                @error('price')<span class="invalid-feedback">{{ $message }}</span>@enderror
                            </div>
                            <div class="col-md-4 form-group">
                                <label class="form-label">Original Price (₹) <span class="text-danger">*</span></label>
                                <input type="number" step="0.01" name="original_price" class="form-control @error('original_price') is-invalid @enderror" value="{{ old('original_price', $reel->original_price) }}">
                                @error('original_price')<span class="invalid-feedback">{{ $message }}</span>@enderror
                            </div>
                            <div class="col-md-4 form-group">
                                <label class="form-label">Discount % <span class="text-danger">*</span></label>
                                <input type="number" name="discount_percent" min="0" max="100" class="form-control @error('discount_percent') is-invalid @enderror" value="{{ old('discount_percent', $reel->discount_percent) }}">
                        ...
                        <div class="row">
                            <div class="col-md-6 form-group">
                                ...
                                <input type="text" name="views_count" class="form-control @error('views_count') is-invalid @enderror" value="{{ old('views_count', $reel->views_count) }}" placeholder="e.g. 2.7K">
                                @error('views_count')<span class="invalid-feedback">{{ $message }}</span>@enderror
                            </div>
                            <div class="col-md-6 form-group">
                                <label class="form-label">Sort Order</label>
                                <input type="number" name="sort_order" min="0" class="form-control @error('sort_order') is-invalid @enderror" value="{{ old('sort_order', $reel->sort_order) }}">
                        ...
                        <div class="form-group">
                            <div class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input" id="is_active" name="is_active" value="1" {{ old('is_active', $reel->is_active) ? 'checked' : '' }}>
                                <label class="custom-control-label" for="is_active">Active (visible on frontend)</label>
                            </div>
                        </div>

                        <div class="d-flex justify-content-end">
                            <button type="submit" class="submit_btn">Update Reel</button>
